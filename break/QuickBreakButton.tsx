...
export function QuickBreakButton() {
    ...
    <>
      {breaksData?.active_break ? (
        <Button
          ...
          className="relative bg-primary/10 border-primary/30 text-primary hover:bg-primary/20 hover:text-primary"
        >
        ...
        <Button
          ...
          className="relative border-border/50 hover:bg-accent/10"
        >
          ...
          {breaksData && (
            <Badge
              variant="outline"
              className={`ml-2 ${
                isDangerLevel(breaksData.normal_total_minutes) ? 'bg-destructive/10 text-destructive border-destructive/20' :
                isWarningLevel(breaksData.normal_total_minutes) ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20' :
                'bg-primary/10 text-primary border-primary/20'
              }`}
            >
              {breaksData.remaining_minutes}m
            </Badge>
          )}
        </Button>
      )}
              ...
              <div className="p-3 bg-muted rounded-lg space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className={`font-semibold ${
                    isDangerLevel(breaksData.normal_total_minutes) ? 'text-destructive' :
                    isWarningLevel(breaksData.normal_total_minutes) ? 'text-amber-500' :
                    'text-foreground'
                  }`}>
                    {breaksData.normal_total_minutes} / 60 min
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Remaining:</span>
                  <span className="font-semibold text-primary">{breaksData.remaining_minutes} min</span>
                </div>
              </div>
            )}
          </div>
